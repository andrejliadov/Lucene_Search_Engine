...
public class QueryInedxCustomScoring
{
            ...
            for(int i = 0; i < querys.get(1).size(); i++) {
            	...
            	for(int j = 0; j < MAX_RESULTS; j++) {
            		...
            		if (scores.get(j).score > 3) {
            			...
            			writer.write(String.valueOf(i + 1) + " Q0 " + (scores.get(j).documentNumber + 1) + " " + (j+1) + " " + utils.mapScore(scores.get(j).score, 3, 52) + " SimpleQueryWithCustomScoring\n");
            		}
            	}
            }            
